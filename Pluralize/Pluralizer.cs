...
using TheRandomizer.Pluralize.Rules;

namespace TheRandomizer.Pluralize;
...
public partial class Pluralizer
{
    ...
    private static readonly Regex replacementRegex = replacementRegexPartial();
    ...
    internal static string Transform(string word, Dictionary<string, string> replacables, Dictionary<string, string> keepables, Dictionary<Regex, string> rules)
    {
        var token = word.ToLower();
        if (keepables.ContainsKey(token)) return RestoreCase(word, token);
        if (replacables.TryGetValue(token, out var value)) return RestoreCase(word, value);
        return ApplyRules(token, word, rules);
    }

    [GeneratedRegex("\\$(\\d{1,2})")]
    private static partial Regex replacementRegexPartial();
}
